...
from dinocrit.core.config import DEFAULT_CACHE_DB, DinocritConfig
from dinocrit.core.interfaces import LLMProvider
from dinocrit.core.logging import get_logger
from dinocrit.core.scoring import ScoreCalculator
from dinocrit.core.types import AnalysisResult, FileContext, Issue, IssueLevel, Location
from dinocrit.core.workspace import GitWorkspaceScanner
from dinocrit.llm.prompts import CriticPromptBuilder
from dinocrit.llm.schemas import CriticResponse
from dinocrit.packs.loader import ComposedPack, PackCompositor
from dinocrit.utils.code import CodeExtractor
...
class Engine:
    ...
    def __init__(self, config: DinocritConfig):
        self.config = config
    ...
    def _create_provider(self) -> LLMProvider:
        ...
        from dinocrit.providers import LiteLLMProvider
    ...
    async def analyze(
        ...
        # 4. Analyze uncached files with LLM
        ...
        if uncached_files and max_calls > 0:
            # Analyze files in parallel with semaphore for rate limiting
            semaphore = asyncio.Semaphore(3)  # Max 3 concurrent calls

            async def analyze_with_limit(file_ctx: FileContext) -> list[Issue]:
                async with semaphore:
                    return await self._analyze_file(file_ctx, composed_pack)

            tasks = [
                analyze_with_limit(file_ctx)
                for file_ctx in uncached_files[:max_calls]
            ]

            results = await asyncio.gather(*tasks, return_exceptions=True)

            for file_ctx, result in zip(uncached_files[:max_calls], results, strict=False):
                if isinstance(result, BaseException):
                    logger.warning("LLM error for %s: %s", file_ctx.path, result)
                    continue

                # result is now guaranteed to be list[Issue]
                issues: list[Issue] = result
