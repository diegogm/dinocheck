...
- Custom YAML rules from .dinocrit/rules/ directory
"""
...
def load_custom_rules(rules_dir: Path | str | None = None) -> list[Rule]:
    ...
    Args:
        rules_dir: Directory containing .yaml rule files.
                   Defaults to .dinocrit/rules/ in current directory.
    ...
    ```
    """
    rules_path = Path.cwd() / ".dinocrit" / "rules" if rules_dir is None else Path(rules_dir)
